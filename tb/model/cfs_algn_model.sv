...
class cfs_algn_model extends uvm_component implements uvm_ext_reset_handler;
  ...
  protected virtual function cfs_md_response get_exp_response(cfs_md_item_mon item);
    ...
    if (item.data.size() == 0) begin   //changed to ==0
      return CFS_MD_ERR;
    end

    //Illegal combination between size and offset: (aligner data width + offset) % size != 0
    if (((env_config.get_algn_data_width() / 8) + item.offset) % item.data.size() != 0) begin  // removed offset-1
      return CFS_MD_ERR;
    end
    ...
    fork
      begin
        ...
        `uvm_info("RX_FIFO", $sformatf(
                  ...
                  reg_block.IRQ.RX_FIFO_FULL.get_mirrored_value()
                  ), UVM_MEDIUM)
    ...
    fork
      begin
        ...
        `uvm_info("RX_FIFO", $sformatf(
                  ...
                  reg_block.IRQ.RX_FIFO_EMPTY.get_mirrored_value()
                  ), UVM_MEDIUM)
    ...
    fork
      begin
        ...
        `uvm_info("TX_FIFO", $sformatf(
                  ...
                  reg_block.IRQ.TX_FIFO_FULL.get_mirrored_value()
                  ), UVM_MEDIUM)
    ...
    fork
      begin
        ...
        `uvm_info("TX_FIFO", $sformatf(
                  ...
                  reg_block.IRQ.TX_FIFO_EMPTY.get_mirrored_value()
                  ), UVM_MEDIUM)
...
int tx_lvl;
  ...
  protected virtual task align();
    ...
    forever begin
      int unsigned ctrl_size = reg_block.CTRL.SIZE.get_mirrored_value();
      int unsigned ctrl_offset = reg_block.CTRL.OFFSET.get_mirrored_value();  //interchanged ctrl.OFFSET and ctrl.SIZE
