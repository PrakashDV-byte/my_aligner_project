 ...
 virtual function void write_in_agent_rx(cfs_md_item_mon item_mon);
    if (!item_mon.is_active()) begin
      /* if (exp_rx_responses.size() == 0) begin
         `uvm_error("DUT_ERROR", "Unexpected RX item - no expected response in queue")
         return;
      end
    
      if (process_exp_rx_response_watchdog.size() == 0) begin
        `uvm_fatal("ALGORITHM_ISSUE", "process_exp_rx_response_watchdog is empty")
      end  */

      cfs_md_response exp_response = exp_rx_responses.pop_front();    //changed to pop front()
    ...
    if (!item_mon.is_active()) begin
      /*	if (exp_tx_items.size() == 0) begin
            `uvm_error("DUT_ERROR", "Unexpected TX item received - no expected item in queue")
         return;
       end

       if (process_exp_tx_item_watchdog.size() == 0) begin
          `uvm_fatal("ALGORITHM_ISSUE", "process_exp_tx_item_watchdog is empty")
       end

       */

      cfs_md_item_mon exp_item = exp_tx_items.pop_front();   //changed to pop front()
  ...
  protected virtual task rcv_irq();
    ...
    forever begin
      ...
      if (exp_irqs.size() == 0) begin
        ...
        void'(process_exp_irq_watchdog.pop_front);     //added brackets for the pop_front().
      end
    end
  endtask
