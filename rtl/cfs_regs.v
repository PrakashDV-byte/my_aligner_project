...
module cfs_regs #(
  ...
  always_comb begin
    ...
    status_rd_val[LSB_STATUS_CNT_DROP+ STATUS_CNT_DROP_WIDTH-1:LSB_STATUS_CNT_DROP] = status_cnt_drop;   //removed +1 here for status_cnt_drop
  ...
  assign irq = 
      ...
      (edge_tx_fifo_full  & irqen_tx_fifo_full);     //added maxdrop in the irq caluculation
              ...
              if (pwrite) begin
                if (wr_ctrl_is_illegal == 0) begin
                  if (ctrl_size_wr_val == 0) begin
                    ...
                    ctrl_clr           <= pwdata[LSB_CTRL_CLR];    //removed the negation here for ~pwdata
                  end
                end else begin
                  wr_ctrl_is_illegal <= 0;
                  pready             <= 1;
