...
module cfs_rx_ctrl #(
  ...
  always_comb begin
    if (md_rx_valid == 1) begin
      if (md_rx_size == 0) begin //removed or  ||md_rx_size==1 in the condition
        md_rx_err = 1;
      end else if ((((ALGN_DATA_WIDTH / 8) + md_rx_offset) % md_rx_size) != 0) begin
        md_rx_err = 1;        //made the md_rx_err =1
      end else begin
        md_rx_err = 0;
      end
    end else begin
  ...
  assign push_data[OFFSET_MSB:OFFSET_LSB] = md_rx_offset;    //  md_rx_offset+1 to just  md_rx_offset
